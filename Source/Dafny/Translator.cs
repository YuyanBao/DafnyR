...
namespace Microsoft.Dafny {
    public class Translator {
        ...
        internal class PredefinedDecls {
            ...
            private readonly Bpl.TypeSynonymDecl regionTypeCtor;  // Yuyan
            ...
            private readonly Bpl.Constant rthisField;     // Yuyan
            ...
            void ObjectInvariant() {
                ...
                Contract.Invariant(regionTypeCtor != null); // Yuyan
            ...
            public Bpl.Type RegionType(IToken tok) {
                ...
                return new Bpl.TypeSynonymAnnotation(Token.NoToken, regionTypeCtor, new Bpl.TypeSeq());
            }
            ...
            // Yuyan
            public Bpl.IdentifierExpr RThis(IToken tok) {
                Contract.Requires(tok != null);
                Contract.Ensures(Contract.Result<Bpl.IdentifierExpr>() != null);

                return new Bpl.IdentifierExpr(tok, rthisField);
            }
            ...
            public PredefinedDecls(Bpl.TypeCtorDecl refType, Bpl.TypeCtorDecl boxType, Bpl.TypeCtorDecl tickType,
                                   ...
                                   Bpl.Constant allocField, Bpl.Constant rthisField /*Yuyan*/, Bpl.Constant classDotArray,
                                   Bpl.TypeSynonymDecl regionTypeCtor /* Yuyan */) {
                ...
                Contract.Requires(regionTypeCtor != null);
                #endregion
        ...
        static PredefinedDecls FindPredefinedDecls(Bpl.Program prog) {
            ...
            Bpl.TypeSynonymDecl regionTypeCtor = null;    // Yuyan
            ...
            Bpl.Constant rthisField = null; // Yuyan
            ...
            if (seqTypeCtor == null) {
                ...
                Console.WriteLine("Error: Dafny prelude is missing declaration of type RegionType"); // Yuyan
            ...
            } else if (rthisField == null) {
                Console.WriteLine("Error: Dafny prelude is missing declaration of constant rthis");
                ...
                return new PredefinedDecls(refType, boxType, tickType,
                                           ...
                                           allocField, rthisField, classDotArray, regionTypeCtor);
            }
            return null;
        }
        ...
        Bpl.Axiom/*!*/ FunctionAxiom(Function/*!*/ f, FunctionAxiomVisibility visibility, Expression body, List<Expression/*!*/>/*!*/ ens, Specialization specialization, int layerOffset) {
            ...
            /*
            // Yuyan: consider pattern variables as formal parameters
            foreach (PatternExpression pe in f.Patterns) {
                bv = new Bpl.BoundVariable(pe.tok, new Bpl.TypedIdent(pe.tok, pe.UniqueName, TrType(pe.Type)));
                formals.Add(bv);
                Bpl.Expr formal = new Bpl.IdentifierExpr(pe.tok, bv);
                args.Add(formal);
                Bpl.Expr wh = GetWhereClause(pe.tok, formal, pe.Type, etran);
                if (wh != null) { ante = Bpl.Expr.And(ante, wh); }
            }
             */
            ...
            if (body == null) {
                ...
                // Yuyan: consider pattern variables as bounded variable in a forall expression
                if (f.Patterns.Count > 0) {
                    Bpl.VariableSeq formalsPattern = new Bpl.VariableSeq();

                    foreach (PatternExpression pe in f.Patterns) {
                        bv = new Bpl.BoundVariable(pe.tok, new Bpl.TypedIdent(pe.tok, pe.tok.val, TrType(pe.Type)));
                        formalsPattern.Add(bv);
                    }


                    meat = new Bpl.ForallExpr(f.tok, new Bpl.TypeVariableSeq(), formalsPattern, null, null, meat);
               }
            }
            if (layerOffset == 0) {
        ...
        // Yuyan
        Bpl.IdentifierExpr GetNewRegionVar_IdExpr(IToken tok, Bpl.VariableSeq locals)  // local variable that's shared between statements that need it
        {
            Contract.Requires(tok != null);
            Contract.Requires(locals != null);
            Contract.Requires(predef != null);
            ...
            // important: the following declaration produces no where clause (that's why we're going through the trouble of setting of this variable in the first place)
            return GetTmpVar_IdExpr(tok, "$nr", predef.RegionType(Token.NoToken), locals);
        }
        ...
        void DefineFrame(IToken/*!*/ tok, List<FrameExpression/*!*/>/*!*/ frameClause, Bpl.StmtListBuilder/*!*/ builder, Bpl.VariableSeq/*!*/ localVariables, string frameName, string footprintName) {
            Contract.Requires(tok != null);
            ...
            Bpl.LocalVariable frame = new Bpl.LocalVariable(tok, new Bpl.TypedIdent(tok, frameName ?? theFrame.Name, theFrame.Type));
        ...
        /// <summary>
        /// Generates:
        ///     axiom (forall<alpha> $o: ref, $f: Field alpha, formals... ::
        ...
        void AddFootprintAxiom(Function f) {
            ...
            Bpl.IdentifierExpr field = new Bpl.IdentifierExpr(f.tok, fieldVar);

            bvars.Add(oVar);
            bvars.Add(fieldVar);
            
            if (!f.IsStatic) {
                Bpl.BoundVariable vThis = new Bpl.BoundVariable(f.tok, new Bpl.TypedIdent(f.tok, "this", predef.RefType));
                bvars.Add(vThis);
                args.Add(new Bpl.IdentifierExpr(f.tok, vThis));
            }

            foreach (Formal p in f.Formals) {
                Bpl.BoundVariable bv = new Bpl.BoundVariable(p.tok, new Bpl.TypedIdent(p.tok, p.UniqueName, TrType(p.Type)));
                bvars.Add(bv);
                Bpl.IdentifierExpr expr = new Bpl.IdentifierExpr(p.tok, bv);
                args.Add(expr);
            }

            string axiomComment = "frame axiom for " + f.FullCompileName + "#footprint";
            Bpl.FunctionCall fn = new Bpl.FunctionCall(new Bpl.IdentifierExpr(f.tok, f.FullCompileName + "#footprint", predef.RegionType(Token.NoToken)));
            Bpl.Expr funCall = new Bpl.NAryExpr(f.tok, fn, args);
            Bpl.Expr funcAppl = Bpl.Expr.Select(funCall, o, field);
            Bpl.Trigger tr = new Bpl.Trigger(f.tok, true, new Bpl.ExprSeq(funcAppl));

            Bpl.Expr frame = InRWRegionClause(f.tok, f.Reads, etran, null, null);
            Bpl.Expr frameSelect;
            if (frame == Bpl.Expr.True) {
                frameSelect = frame;
            } else {
                frameSelect = Bpl.Expr.Select(frame, o, field);
            }
            Bpl.Expr body = Bpl.Expr.Iff(funcAppl, frameSelect);

            //Bpl.TypeVariableSeq typeParams = TrTypeParamDecls(f.TypeArgs);
            //typeParams.Add(new Bpl.TypeVariableSeq(alpha));
            Bpl.Expr ax = new Bpl.ForallExpr(f.tok, new Bpl.TypeVariableSeq(alpha), bvars, null, tr, body);
        ...
        void AddFrameAxiom(Function f) {
            ...
            Bpl.Expr r0 = InRWRegionClause(f.tok, o, field, f.Reads, etran0, null, null);
        ...
        Bpl.Expr/*!*/ InRWClause(IToken/*!*/ tok, Bpl.Expr/*!*/ o, Bpl.Expr/*!*/ f, List<FrameExpression/*!*/>/*!*/ rw, ExpressionTranslator/*!*/ etran,
            ...
            foreach (FrameExpression rwComponent in rw) {
                Expression e = rwComponent.E.Resolved;
                if (receiverReplacement != null) {
        ...
        Bpl.Expr/*!*/ InRWRegionClause(IToken/*!*/ tok, List<FrameExpression/*!*/>/*!*/ rw, ExpressionTranslator/*!*/ etran,
            ...
            Bpl.Expr emptyRegion = FunctionCall(tok, BuiltinFunction.RegionEmpty, null);
            Bpl.Expr union = emptyRegion;
            foreach (FrameExpression rwComponent in rw) {
                ...
                if (e is RegionConstructExpression) {
                    RegionConstructExpression rce = (RegionConstructExpression)e;
                    if (rce.RegionList == null) {
                        // should not be, there must be some elements
                        // maybe missed in the resolution
                        Contract.Assert(false); throw new cce.UnreachableException();
                    } else if (rce.RegionList.Count == 0) {
                        // empty region
                        // nothing to do
                    } else {
                        if (rce.canUseRThis) {
                            Contract.Assert(rce.RegionList.Count > 0);
                            Expression obj = rce.RegionList[0].Obj;
                            if (receiverReplacement != null) {
                                Contract.Assert(substMap != null);
                ...
                } else if (e is FootprintExpression) {
                    FootprintExpression fe = (FootprintExpression)e;
                    Bpl.Expr regionlst;
                    foreach (RegionConstructExpression rce in fe.Footprint) {

                        Expression expr = rce.E.Resolved;
                        if (expr is FunctionCallExpr) {
                            FunctionCallExpr fce = (FunctionCallExpr)expr;
                            Contract.Assert(fce.Function != null);  // follows from the fact that expr has been successfully resolved

                            Bpl.IdentifierExpr footprintFuncID = new Bpl.IdentifierExpr(fce.tok, fce.Function.FullCompileName + "#footprint", predef.RegionType(Token.NoToken));
                            ExprSeq args = etran.FunctionInvocationArguments(fce);
                            Bpl.Expr footprintFuncApp = new Bpl.NAryExpr(fce.tok, new Bpl.FunctionCall(footprintFuncID), args);

                            regionlst = etran.translator.FunctionCall(e.tok, BuiltinFunction.RegionUnion, null, union, footprintFuncApp);
                            union = FunctionCall(e.tok, BuiltinFunction.RegionUnion, null, union, regionlst);
                        } else {
                            regionlst = TrRegionList(rce.tok, rce.RegionList, etran, receiverReplacement, substMap);
                            union = FunctionCall(e.tok, BuiltinFunction.RegionUnion, null, union, regionlst);
                        }
                    }
                    ...
                    //union = FunctionCall(e.tok, BuiltinFunction.RegionUnion, null, union, etran.TrExpr(e));
                }
       ...
       Bpl.Expr TrRegionFilterExpression(RegionFilterExpression rfe, ExpressionTranslator/*!*/ etran) {
        ...
        Bpl.Expr/*!*/ InRWRegionClause(IToken/*!*/ tok, Bpl.Expr/*!*/ o, Bpl.Expr/*!*/ f, List<FrameExpression/*!*/>/*!*/ rw, ExpressionTranslator/*!*/ etran,
            ...
            if (footprint == Bpl.Expr.True) {
                ...
                ret = Bpl.Expr.Select(footprint, o, f);
            }
            return ret;
        }

        // Yuyan
        Bpl.Expr TrRegionList(IToken/*!*/ tok, List<Region> regionList, ExpressionTranslator/*!*/ etran,
                Expression receiverReplacement, Dictionary<IVariable, Expression/*!*/> substMap) {
            Contract.Requires(tok != null);
            Contract.Requires(regionList != null);
            Contract.Requires(etran != null);
            Contract.Requires(substMap == null || cce.NonNullDictionaryAndValues(substMap));
            Contract.Requires((receiverReplacement == null) == (substMap == null));
            Contract.Ensures(Contract.Result<Bpl.Expr>() != null);
            ...
            foreach (Region r in regionList) {
                ...
                //if (receiverReplacement != null) {
                //    Contract.Assert(substMap != null);
                //    obj = Substitute(r.Obj, receiverReplacement, substMap);
                //}
                Bpl.Expr rr;
                ...
                if (r.Field.Name == "*") {
                    ...
                    Bpl.Expr f = new Bpl.IdentifierExpr(r.Field.tok, GetField(r.Field));
                    //if (r.Field.Type is RegionType) {
                    //    rr = ExpressionTranslator.ReadHeap(r.Field.tok, etran.HeapExpr, obj, f);
                    //} else {
                    rr = FunctionCall(r.Field.tok, BuiltinFunction.RegionSingleton, null, objExpr, f);
                    //}
                }
                union = FunctionCall(obj.tok, BuiltinFunction.RegionUnion, null, union, rr);
            }
        ...
        void AddWellformednessCheck(Function f) {
            ...
            /*
            // Yuyan: consider pattern variables as formals
            foreach (PatternExpression pe in f.Patterns) {
                Bpl.Type varType = TrType(pe.Type);
                Bpl.Expr wh = GetWhereClause(pe.tok, new Bpl.IdentifierExpr(pe.tok, pe.UniqueName, varType), pe.Type, etran);
                inParams.Add(new Bpl.Formal(pe.tok, new Bpl.TypedIdent(pe.tok, pe.UniqueName, varType, wh), true));
            }
             */
            Bpl.TypeVariableSeq typeParams = TrTypeParamDecls(f.TypeArgs);
            ...
            // Yuyan: consider pattern variables as locals
            foreach (PatternExpression pe in f.Patterns) {
                if (pe.E.Resolved is IdentifierExpr) {
                    IdentifierExpr ie = (IdentifierExpr)pe.E.Resolved;
                    BoundVariable bv = new Bpl.BoundVariable(pe.tok, new Bpl.TypedIdent(ie.tok, ie.Var.UniqueName, TrType(ie.Type)));
                    locals.Add(bv);
                }
            }
            ...
            if (f.Body == null) {
                ...
                DefineFrame(f.tok, f.Reads, bodyCheckBuilder, locals, null, null);
        ...
        Bpl.Expr IsTotal(Expression expr, ExpressionTranslator etran) {
            ...
            if (expr is LiteralExpr || expr is ThisExpr || expr is IdentifierExpr || expr is WildcardExpr || expr is BoogieWrapper) {
                ...
                if (e.ResolvedOp == UnaryExpr.ResolvedOpcode.SetChoose) {
                ...
                } else if (e.ResolvedOp == UnaryExpr.ResolvedOpcode.SeqLength && (!(e.E.Type is SeqType) || !(e.E.Type is RegionType))) {
        ...
        Bpl.Expr CanCallAssumption(Expression expr, ExpressionTranslator etran) {
            ...
            if (expr is LiteralExpr || expr is ThisExpr || expr is IdentifierExpr || expr is WildcardExpr || expr is BoogieWrapper || expr is PatternExpression /* Yuyan */) {
                return Bpl.Expr.True;
            } else if (expr is DisplayExpression) {
                DisplayExpression e = (DisplayExpression)expr;
                ...
                return CanCallAssumption(rce.E, etran);
            } else if (expr is FootprintExpression) { // Yuyan
                FootprintExpression fe = (FootprintExpression)expr;
                return CanCallAssumption(fe.E, etran);
            } else if (expr is RegionFilterExpression){
                RegionFilterExpression rfe = (RegionFilterExpression)expr;
                return CanCallAssumption(rfe.regionE, etran);
            } else if (expr is DatatypeValue) {
                DatatypeValue dtv = (DatatypeValue)expr;
            ...
            }else if (expr is BuiltInFunctionExpression){ // Yuyan
                var e  = (BuiltInFunctionExpression)expr;
                Bpl.Expr total = Bpl.Expr.True;
                foreach (Expression ee in e.Args)
                {
                    total = BplAnd(total, CanCallAssumption(ee, etran));
                }
                return total;
            } else {
                Contract.Assert(false); throw new cce.UnreachableException();  // unexpected expression
        ...
        void CheckWellformedWithResult(Expression expr, WFOptions options, Bpl.Expr result, Type resultType,
            ...
            if (expr is LiteralExpr || expr is ThisExpr || expr is IdentifierExpr || expr is WildcardExpr || expr is BoogieWrapper || expr is PatternExpression /* Yuyan */) {
                ...
                } else if (e.ResolvedOp == UnaryExpr.ResolvedOpcode.SeqLength && !(e.E.Type is SeqType)) {
                ...
                Contract.Assert(e.RegionList != null);
                foreach (Region r in e.RegionList) {
                    CheckWellformedWithResult(r.Obj, options, result, resultType, locals, builder, etran);
                }
            } else if (expr is FootprintExpression) { // Yuyan
                var e = (FootprintExpression)expr;
                foreach (Expression ee in e.Footprint) {
                    CheckWellformedWithResult(ee, options, result, resultType, locals, builder, etran);
                }
            } else if (expr is RegionFilterExpression){ // Yuyan: don't have to check class and field expressions
                var e = (RegionFilterExpression)expr;
                CheckWellformedWithResult(e.regionE, options, result, resultType, locals, builder, etran);
            }
            else if (expr is BuiltInFunctionExpression)
            {
                var e = (BuiltInFunctionExpression)expr;
                foreach (Expression ee in e.Args)
                {
        ...
        List<Expression> FunctionDecreasesWithDefault(Function f, out bool inferredDecreases) {
            ...
            if (decr.Count == 0) {
                ...
                if (f.Reads.Count == 0) {
                    ...
                    decr.Add(RegionFrameUnions(f.Reads));
                }
            }
            return decr;
        }
        ...
        // Yuyan
        Expression RegionFrameUnions(List<FrameExpression> fexprs) {
            Contract.Requires(fexprs != null);
            Contract.Ensures(Contract.Result<Expression>() != null);

            List<Expression> rlist = new List<Expression>();
            foreach (FrameExpression fe in fexprs) {
                Expression e = fe.ResolvedExpression;
                Contract.Assert(e != null);
                rlist.Add(e);
            }
            if (rlist.Count == 0) {
                Expression emptyset = new SetDisplayExpr(Token.NoToken, new List<Expression>()); // TBD: maybe incorrect
                emptyset.Type = new SetType(new RegionType());  // resolve here
                return emptyset;
            } else {
                Expression s = rlist[0];
                for (int i = 1; i < rlist.Count; i++) {
                    BinaryExpr union = new BinaryExpr(s.tok, BinaryExpr.Opcode.Add, s, rlist[i]);
                    union.ResolvedOp = BinaryExpr.ResolvedOpcode.RegionUnion;  // resolve here
                    union.Type = new RegionType();  // resolve here
                    s = union;
                }
                return s;
            }
                ...
                if (fe.E is WildcardExpr) {
                    ...
                    if (e.Type.IsRefType) {
                        ...
                        if (e.Type is SeqType) {
                            ...
                            // Yuyan: comment out below to allow e.Type be any type
                            //// e is already a set
        ...
        void AddFunction(Function f) {
            ...
            Bpl.TypeVariableSeq typeParams = TrTypeParamDecls(f.TypeArgs);
            Bpl.VariableSeq args = new Bpl.VariableSeq();
            Bpl.Formal heap = new Bpl.Formal(f.tok, new Bpl.TypedIdent(f.tok, "$heap", predef.HeapType), true);
            args.Add(heap);
            if (!f.IsStatic) {
            ...
            /*
            // Yuyan: consider patter varialbes as InParamas
            foreach (PatternExpression p in f.Patterns) {
                Bpl.Type varType = TrType(p.Type);
                args.Add(new Bpl.Formal(p.tok, new Bpl.TypedIdent(p.tok, p.UniqueName, varType), true));
            }
             */
            ...
            // Yuyan
            Bpl.Formal resftp = new Bpl.Formal(f.tok, new Bpl.TypedIdent(f.tok, Bpl.TypedIdent.NoName, predef.RegionType(Token.NoToken)), false);
            Bpl.VariableSeq fpArgs = new VariableSeq(args);
            Bpl.Function footprintF = new Bpl.Function(f.tok, f.FullCompileName + "#footprint", fpArgs, resftp);
            sink.TopLevelDeclarations.Add(footprintF);
        }
        ...
        Bpl.Procedure AddMethod(Method m, int kind, bool isRefinementMethod) {
            ...
            Dictionary<String, Expression> funDst = new Dictionary<string, Expression>();
            if (kind != 0) {
                string comment = "user-defined preconditions";
                foreach (MaybeFreeExpression p in m.Req) {
                    // Yuyan: consider pattern varialbes in functions of specs as InParamas
                    if (p.E.Resolved is FunctionCallExpr) {
                        FunctionCallExpr fce = (FunctionCallExpr)p.E.Resolved;
                        if (!funDst.ContainsKey(fce.Function.Name)) {
                            funDst.Add(fce.Function.Name, fce);
                            m.Patterns.AddRange(fce.Function.Patterns);
                            foreach (PatternExpression pe in fce.Function.Patterns) {
                                if (pe.E.Resolved is IdentifierExpr) {
                                    IdentifierExpr ie = (IdentifierExpr)pe.E.Resolved;
                                    Bpl.Type varType = TrType(pe.Type);
                                    Bpl.Expr wh = GetWhereClause(pe.tok, new Bpl.IdentifierExpr(ie.tok, ie.Var.UniqueName, varType), pe.Type, etran);
                                    inParams.Add(new Bpl.Formal(pe.tok, new Bpl.TypedIdent(ie.tok, ie.Var.UniqueName, varType, wh), true));
                                }
                            }
                        }
                ...
                foreach (MaybeFreeExpression p in m.Ens) {
                    /* 
                    // Yuyan: consider pattern variables as inParams of the method
                    // do not try to do that; there is a better way to do that
                    if (p.E.Resolved is FunctionCallExpr) {
                        FunctionCallExpr fce = (FunctionCallExpr)p.E.Resolved;
                        if (!funDst.ContainsKey(fce.Function.Name)) {
                            funDst.Add(fce.Function.Name, fce);
        ...
        private void GenerateMethodParametersChoose(IToken tok, ICodeContext m, bool includeReceiver, bool includeInParams, bool includeOutParams,
          ExpressionTranslator etran, out Bpl.VariableSeq inParams, out Bpl.VariableSeq outParams) {
            ...
            // Yuyan: consider patter varialbes as InParamas
            foreach (PatternExpression p in m.Patterns) {
                if (p.E.Resolved is IdentifierExpr) {
                    IdentifierExpr ie = (IdentifierExpr)p.E.Resolved;
                    Bpl.Type varType = TrType(p.Type);
                    Bpl.Expr wh = GetWhereClause(p.tok, new Bpl.IdentifierExpr(ie.tok, ie.Var.UniqueName, varType), ie.Type, etran);
                    inParams.Add(new Bpl.Formal(p.tok, new Bpl.TypedIdent(ie.tok, ie.Var.UniqueName, varType, wh), true));
                }
            }
        ...
        Bpl.Expr/*!*/ FrameCondition(IToken/*!*/ tok, List<FrameExpression/*!*/>/*!*/ modifiesClause, ExpressionTranslator/*!*/ etranPre, ExpressionTranslator/*!*/ etran, ExpressionTranslator/*!*/ etranMod) {
            ...
            Bpl.Expr footprint = InRWRegionClause(tok, modifiesClause, etranMod, null, null);
            Bpl.Expr fieldsinfootprint;
            if (footprint == Bpl.Expr.True) {
                fieldsinfootprint = footprint;
            } else {
                fieldsinfootprint = Bpl.Expr.Select(footprint, o, f);
            }
            consequent = Bpl.Expr.Or(consequent, fieldsinfootprint);
        ...
        Bpl.Type TrType(Type type) {
            ...
            if (type is BoolType) {
                ...
                return Bpl.Type.Int;
            } else if (type is RegionType) { // Yuyan
                return predef.RegionType(Token.NoToken);
        ...
        // Yuyan: Generate pattern variables
        void GeneratePatternVariables(Method m, List<PatternExpression/*!*/>/*!*/ patterns, Bpl.StmtListBuilder/*!*/ builder, Bpl.VariableSeq localVariables) {
            Contract.Requires(m != null);
            Contract.Requires(patterns != null);
            Contract.Requires(localVariables != null);
            Bpl.IdentifierExprSeq varSeq = new IdentifierExprSeq();
            foreach (PatternExpression p in patterns) {
                if (p.E.Resolved is IdentifierExpr) {
                    IdentifierExpr ie = (IdentifierExpr)p.E.Resolved;
                    Bpl.IdentifierExpr var = new Bpl.IdentifierExpr(ie.tok, ie.Var.UniqueName, TrType(ie.Type));
                    localVariables.Add(new LocalVariable(p.tok, new Bpl.TypedIdent(ie.tok, ie.Var.UniqueName, TrType(ie.Type))));
                    varSeq.Add(var);
                }
            }

            builder.Add(new Bpl.HavocCmd(m.tok, varSeq));
        }
        ...
        void TrLoop(LoopStmt s, Expression Guard, BodyTranslator bodyTr,
            ...
            string loopFootPrintName = "#_FootPrint#" + loopId;
        ...
        void ProcessCallStmt(IToken tok,
           ...
           Bpl.StmtListBuilder builder, Bpl.VariableSeq locals, ExpressionTranslator etran) {
            ...
            // Dictionary<IVariable, Expression> substMapFun = new Dictionary<IVariable, Expression>();
            // Yuyan: consider pattern variables as In-parameters
            foreach (PatternExpression pe in method.Patterns) {
                if (pe.E.Resolved is IdentifierExpr) {
                    IdentifierExpr ie = (IdentifierExpr)pe.E.Resolved;
                    VarDecl plv = new VarDecl(ie.tok, ie.Var.Name + "#", ie.Var.Type, true);
                    plv.type = plv.OptionalType;
                    IdentifierExpr pie = new IdentifierExpr(plv.Tok, plv.UniqueName);
                    pie.Var = plv; pie.Type = pie.Var.Type;
                    substMap.Add(ie.Var, pie);
                    locals.Add(new Bpl.LocalVariable(plv.Tok, new Bpl.TypedIdent(plv.Tok, plv.UniqueName, TrType(plv.Type))));
                    ins.Add((Bpl.IdentifierExpr)etran.TrExpr(pie));
                }
            }

            // Yuyan: assume free-precondition, this is dangerous
            // TODO: need to handle the following case:
            // predicate valid(n: Node, d: int, reg: RegionType) {
            //      n != null ==> d == n.data && reg == region{n.*}
            // }
            // requires valid(n, d, ?reg);
            // Which translates to
            //      assert d == n.data;
            //      assume reg == region{n.*}
            ExpressionTranslator etranF = new ExpressionTranslator(this, predef, etran.HeapExpr);
            foreach (MaybeFreeExpression mfe in method.Req) {
                if (mfe.IsFree) {
                    Expression e = Substitute(mfe.E, receiver, substMap);
                    if (e.Resolved is FunctionCallExpr) {
                        FunctionCallExpr fce = (FunctionCallExpr)e.Resolved;
                        Bpl.IdentifierExpr funID = new Bpl.IdentifierExpr(fce.tok, fce.Function.FullCompileName, TrType(fce.Function.ResultType));
                        ExprSeq args = etranF.FunctionInvocationArguments(fce);

                        builder.Add(new Bpl.AssumeCmd(fce.tok, new Bpl.NAryExpr(fce.tok, new Bpl.FunctionCall(funID), args)));
                    } else {
                        builder.Add(new Bpl.AssumeCmd(mfe.E.tok, etran.TrExpr(e)));
                    }
                }
            }
        ...
        Bpl.Expr GetWhereClause(IToken tok, Bpl.Expr x, Type type, ExpressionTranslator etran) {
            ...
            if (type is NatType) {
            ...
            } else if (type is BoolType || type is IntType || type is RegionType /* Yuyan */) {
                // nothing to do
                ...
                return Bpl.Expr.Or(Bpl.Expr.Eq(x, predef.Null), Bpl.Expr.And(etran.GoodRef(tok, x, type), etran.GoodRThis(tok, x, type)));
        ...
        Bpl.IdentifierExpr TrAssignmentRhs(IToken tok, Bpl.IdentifierExpr bLhs, Type lhsType, AssignmentRhs rhs, Type checkSubrangeType,
            ...
            if (rhs is ExprRhs) {
                ...
                if (ch != null && ch.ResolvedOp == UnaryExpr.ResolvedOpcode.SetChoose) {
                    // havoc $Tick;
                ...
                Bpl.IdentifierExpr rw = GetNewRegionVar_IdExpr(tok, locals); // Yuyan
                ...
                // Yuyan
                // region begins
                builder.Add(new Bpl.HavocCmd(tok, new Bpl.IdentifierExprSeq(rw)));

                // Yuyan: update rthis as all the fields 
                Bpl.Expr fieldsUnion = UnionFields(tRhs, nw);
                builder.Add(Bpl.Cmd.SimpleAssign(Token.NoToken, rw, fieldsUnion));

                Bpl.IdentifierExpr heap = (Bpl.IdentifierExpr/*TODO: this cast is dubious*/)etran.HeapExpr;

                // assume Region#Disjoint(rw, Region);
                Bpl.Expr gRegion = FunctionCall(Token.NoToken, BuiltinFunction.RegionOfHeap, null, heap);
                builder.Add(new Bpl.AssumeCmd(tok, FunctionCall(Token.NoToken, BuiltinFunction.RegionDisjoint, null, rw, gRegion)));


                Bpl.Cmd rthisupdate = Bpl.Cmd.SimpleAssign(tok, heap, ExpressionTranslator.UpdateHeap(tok, heap, nw, predef.RThis(tok), rw));
                builder.Add(rthisupdate);

                // region ends

                // $Heap[$nw, alloc] := true;
                Bpl.Expr alloc = predef.Alloc(tok);
                //Yuyan, it is defined above:  Bpl.IdentifierExpr heap = (Bpl.IdentifierExpr/*TODO: this cast is dubious*/)etran.HeapExpr;
        ...
        // Yuyan
        Bpl.Expr UnionFields(Expression objExpr, ExpressionTranslator/*!*/ etran) {
            Contract.Requires(objExpr != null);
            Contract.Requires(etran != null);
            Contract.Ensures(Contract.Result<Bpl.Expr>() != null);


            Bpl.Expr obj = etran.TrExpr(objExpr);
            Bpl.Expr result = null;
            Bpl.Expr singleRegion = null;
            Contract.Assert(objExpr.Type != null);

            UserDefinedType udt = (UserDefinedType)objExpr.Type;
            ClassDecl cd = (ClassDecl)udt.ResolvedClass;
            foreach (MemberDecl member in cd.Members) {
                if (member is Field) {
                    singleRegion = FunctionCall(Token.NoToken, BuiltinFunction.RegionSingleton, null, obj, new Bpl.IdentifierExpr(Token.NoToken, GetField((Field)member)));

                    if (result == null) {
                        result = singleRegion;
                    } else {
                        result = FunctionCall(Token.NoToken, BuiltinFunction.RegionUnion, null, result, singleRegion);
                    }
                }
            }
        ...
        // Yuyan
        Bpl.Expr UnionFields(TypeRhs type, Bpl.IdentifierExpr nw) {
            Contract.Requires(type != null);
            ...
            if (type.ArrayDimensions != null) {
                // TODO
            } else if (type.EType is ObjectType) {
                // TODO
            } else {
                UserDefinedType udt = (UserDefinedType)type.EType;
                ClassDecl cd = (ClassDecl)udt.ResolvedClass;
                foreach (MemberDecl member in cd.Members) {
                    if (member is Field) {
                        singleRegion = FunctionCall(Token.NoToken, BuiltinFunction.RegionSingleton, null, nw, new Bpl.IdentifierExpr(Token.NoToken, GetField((Field)member)));
        ...
        internal class ExpressionTranslator {
            ...
            ExpressionTranslator(Translator translator, PredefinedDecls predef, Bpl.Expr heap, string thisVar,
              Function applyLimited_CurrentFunction, int layerOffset, string modifiesFrame, string modifiesFootPrint/* Yuyan */) {
                ...
                Contract.Requires(modifiesFootPrint != null); /* Yuyan */
            ...
            public ExpressionTranslator(Translator translator, PredefinedDecls predef, Bpl.Expr heap, string thisVar)
                : this(translator, predef, heap, thisVar, null, 0, "$_Frame", "$_FootPrint"/* Yuyan */) {
                Contract.Requires(translator != null);
                Contract.Requires(predef != null);
                Contract.Requires(heap != null);
                Contract.Requires(thisVar != null);
            ...
            public ExpressionTranslator(ExpressionTranslator etran, string modifiesFrame, string modifiesFootPrint /* Yuyan */)
                : this(etran.translator, etran.predef, etran.HeapExpr, etran.This, etran.applyLimited_CurrentFunction, etran.layerOffset, modifiesFrame, modifiesFootPrint /* Yuyan */) {
                Contract.Requires(etran != null);
                Contract.Requires(modifiesFrame != null);
            ...
            public ExpressionTranslator Old {
                get {
                    ...
                    if (oldEtran == null) {
                        oldEtran = new ExpressionTranslator(translator, predef, new Bpl.OldExpr(HeapExpr.tok, HeapExpr), This, applyLimited_CurrentFunction, layerOffset, modifiesFrame, modifiesFootPrint /* Yuyan */);
            ...
            public ExpressionTranslator LimitedFunctions(Function applyLimited_CurrentFunction) {
                ...
                return new ExpressionTranslator(translator, predef, HeapExpr, This, applyLimited_CurrentFunction, layerOffset, modifiesFrame, modifiesFootPrint/* Yuyan */);
            }
            ...
            public ExpressionTranslator LayerOffset(int offset) {
                ...
                var et = new ExpressionTranslator(translator, predef, HeapExpr, This, applyLimited_CurrentFunction, layerOffset + offset, modifiesFrame, modifiesFootPrint/* Yuyan */);
                if (this.oldEtran != null) {
                    var etOld = new ExpressionTranslator(translator, predef, Old.HeapExpr, This, applyLimited_CurrentFunction, layerOffset + offset, modifiesFrame, modifiesFootPrint /* Yuyan */);
            ...
            public Bpl.Expr TrExpr(Expression expr) {
                ...
                if (expr is LiteralExpr) {
                ...
                } else if (expr is PatternExpression) { // Yuyan
                    PatternExpression e = (PatternExpression)expr;
                    return TrExpr(e.E);
                    ...
                    FreshExpr e = (FreshExpr)expr;
                    if (e.E.Type is RegionType) {
                        // Yuyan
                        // genertae assume (forall<alpha> $o: ref, $f: Field alpha :: 
                        //  Tr(expr.E)[$o, $f] ==> !read(old($Heap), $o, alloc));
                        Bpl.Expr ee = TrExpr(e.E);
                        Bpl.TypeVariable alpha = new Bpl.TypeVariable(expr.tok, "alpha");
                        ...
                        Bpl.BoundVariable fVar = new Bpl.BoundVariable(expr.tok, new Bpl.TypedIdent(expr.tok, "$f", predef.FieldName(expr.tok, alpha)));
                        Bpl.Expr f = new Bpl.IdentifierExpr(expr.tok, fVar);
                        Bpl.Expr ofInRegion = Bpl.Expr.Select(ee, o, f);
                        Bpl.Expr oIsFresh = Bpl.Expr.Not(Old.IsAlloced(expr.tok, o));
                        Bpl.Expr body = Bpl.Expr.Imp(ofInRegion, oIsFresh);
                        return new Bpl.ForallExpr(expr.tok, new Bpl.TypeVariableSeq(alpha), new Bpl.VariableSeq(oVar, fVar), body);
                    ...
                    switch (e.ResolvedOp) {
                        case UnaryExpr.ResolvedOpcode.Not:
                            return Bpl.Expr.Unary(expr.tok, UnaryOperator.Opcode.Not, arg);
                        case UnaryExpr.ResolvedOpcode.SetChoose:
                            ...
                            return x;
                        case UnaryExpr.ResolvedOpcode.SeqLength:
                        ...
                        case UnaryExpr.ResolvedOpcode.RegionLength:
                            return translator.FunctionCall(expr.tok, BuiltinFunction.RegionSize, null, arg);
                        default:
                    ...
                    if (e.ResolvedOp == BinaryExpr.ResolvedOpcode.InSet) {
                        ...
                        return Bpl.Expr.Unary(expr.tok, UnaryOperator.Opcode.Not, arg);
                    } else if (e.ResolvedOp == BinaryExpr.ResolvedOpcode.InRegion) {   // Yuyan
                        Bpl.Expr arg = TrInRegion(expr.tok, e0, e.E1, cce.NonNull(e.E0.Type));
                    }
                    ...
                    switch (e.ResolvedOp) {
                        ...
                        // Yuyan
                        case BinaryExpr.ResolvedOpcode.RegionEq:
                            return translator.FunctionCall(expr.tok, BuiltinFunction.RegionEqual, null, e0, e1);
                        case BinaryExpr.ResolvedOpcode.RegionNeq:
                            return Bpl.Expr.Not(translator.FunctionCall(expr.tok, BuiltinFunction.RegionEqual, null, e0, e1));
                        case BinaryExpr.ResolvedOpcode.RegionUnion:
                            return translator.FunctionCall(expr.tok, BuiltinFunction.RegionUnion, null, e0, e1);
                        case BinaryExpr.ResolvedOpcode.RegionSubRegion:
                            return translator.FunctionCall(expr.tok, BuiltinFunction.RegionSubRegion, null, e0, e1);
                        case BinaryExpr.ResolvedOpcode.RegionSubOrEqualRegion:
                            Bpl.Expr t1 = translator.FunctionCall(expr.tok, BuiltinFunction.RegionSubRegion, null, e0, e1);
                            Bpl.Expr t2 = translator.FunctionCall(expr.tok, BuiltinFunction.RegionEqual, null, e0, e1);
                            return Bpl.Expr.Binary(expr.tok, BinaryOperator.Opcode.Or, t1, t2);
                        case BinaryExpr.ResolvedOpcode.RegionDisjoint:
                            return translator.FunctionCall(expr.tok, BuiltinFunction.RegionDisjoint, null, e0, e1);
                        case BinaryExpr.ResolvedOpcode.RegionDifference:
                            return translator.FunctionCall(expr.tok, BuiltinFunction.RegionDifference, null, e0, e1);
                        case BinaryExpr.ResolvedOpcode.RegionIntersection:
                            return translator.FunctionCall(expr.tok, BuiltinFunction.RegionIntersection, null, e0, e1);
                        case BinaryExpr.ResolvedOpcode.InRegion:
                            if (e.E0.Resolved is FieldSelectExpr) {
                                FieldSelectExpr fs = (FieldSelectExpr)e.E0.Resolved;
                                Bpl.Expr obj = TrExpr(fs.Obj);
                                Bpl.Expr field = new Bpl.IdentifierExpr(fs.tok, translator.GetField(fs.Field));
                                return Bpl.Expr.SelectTok(fs.tok, e1, obj, field);
                            } else {
                                Contract.Assert(false); throw new cce.UnreachableException();
                            }
                    ...
                    BuiltInFunctionExpression e = (BuiltInFunctionExpression)expr;
                    return TrBuiltInFunctionExpr(e.tok, e.FunctionName, e.Args, this);
                } else {
                    Contract.Assert(false); throw new cce.UnreachableException();  // unexpected expression
                }
            }
            ...
            // Yuyan
            Bpl.Expr TrBuiltInFunctionExpr(IToken/*!*/ tok, string funName, List<Expression> args, ExpressionTranslator etran){
                Contract.Requires(tok != null);
                Contract.Requires(funName != null);
                Contract.Requires(args != null);
                Contract.Ensures(Contract.Result<Bpl.Expr>() != null);

                if (funName == "IsRecursiveType")
                {
                    Contract.Assert(args.Count == 1);
                    return translator.FunctionCall(tok, BuiltinFunction.RecursiveType, null, etran.HeapExpr, TrExpr(args[0]));
                }
                else
                {
                    Contract.Assert(false); throw new cce.UnreachableException();  // unexpected expression
            ...
            // Yuyan
            Bpl.Expr TrFootprint(IToken/*!*/ tok, List<RegionConstructExpression> expressionList, ExpressionTranslator etran) {
                Contract.Requires(tok != null);
                Contract.Requires(expressionList != null);
                Contract.Requires(etran != null);
                Contract.Ensures(Contract.Result<Bpl.Expr>() != null);
                Bpl.Expr emptyRegion = etran.translator.FunctionCall(tok, BuiltinFunction.RegionEmpty, null);
                Bpl.Expr ret = emptyRegion;
                Bpl.Expr rr = emptyRegion;
                foreach (RegionConstructExpression rce in expressionList) {
                    Expression expr = rce.E.Resolved;
                    if (expr is FunctionCallExpr) {
                        FunctionCallExpr e = (FunctionCallExpr)expr;
                        Contract.Assert(e.Function != null);  // follows from the fact that expr has been successfully resolved

                        Bpl.IdentifierExpr footprintFuncID = new Bpl.IdentifierExpr(e.tok, e.Function.FullCompileName + "#footprint", predef.RegionType(Token.NoToken));
                        ExprSeq args = etran.FunctionInvocationArguments(e);
                        Bpl.Expr footprintFuncApp = new Bpl.NAryExpr(e.tok, new Bpl.FunctionCall(footprintFuncID), args);

                        ret = etran.translator.FunctionCall(e.tok, BuiltinFunction.RegionUnion, null, ret, footprintFuncApp);
                        //// create a local variable for each formal parameter, and assign each actual parameter to the corresponding local
                        //Dictionary<IVariable, Expression> substMap = new Dictionary<IVariable, Expression>();
                        //for (int i = 0; i < e.Function.Formals.Count; i++) {
                        //    substMap.Add(e.Function.Formals[i], e.Args[i].Resolved);
                        //}


                        //foreach (FrameExpression fe in e.Function.Reads) {
                        //    Expression ee = fe.ResolvedExpression;
                        //    if (ee is RegionConstructExpression) {
                        //        RegionConstructExpression rcee = (RegionConstructExpression)ee;
                        //        if (rcee.RegionList == null) {
                        //            // should not be, there must be some elements
                        //            // maybe missed in the resolution
                        //            Contract.Assert(false); throw new cce.UnreachableException();
                        //        } else if (rcee.RegionList.Count == 0) {
                        //            // empty region
                        //            // nothing to do
                        //        } else {
                        //            if (rcee.canUseRThis) {
                        //                Contract.Assert(rcee.RegionList.Count > 0);
                        //                Expression obj = rcee.RegionList[0].Obj;
                        //                if (e.Receiver != null) {
                        //                    Contract.Assert(substMap != null);
                        //                    obj = Substitute(obj, e.Receiver, substMap);
                        //                }
                        //                rr = etran.translator.FunctionCall(e.tok, BuiltinFunction.RegionOfRef, null, etran.HeapExpr, etran.TrExpr(obj));
                        //                ret = etran.translator.FunctionCall(expr.tok, BuiltinFunction.RegionUnion, null, ret, rr);
                        //            } else {
                        //                Bpl.Expr regionlst = etran.translator.TrRegionList(e.tok, rcee.RegionList, etran, e.Receiver, substMap);
                        //                ret = etran.translator.FunctionCall(e.tok, BuiltinFunction.RegionUnion, null, ret, regionlst);
                        //            }
                        //        }
                        //    } else {
                        //        ret = etran.translator.FunctionCall(e.tok, BuiltinFunction.RegionUnion, null, ret, etran.TrExpr(e));
                        //    }

                        //}
                    } else {
                        rr = etran.translator.TrRegionList(rce.tok, rce.RegionList, etran, null, null);
                        ret = etran.translator.FunctionCall(expr.tok, BuiltinFunction.RegionUnion, null, ret, rr);
            ...
            // Yuyan
            private List<Region> GenerateRegionList(Expression e) {
                Contract.Requires(e != null);
            ...
            public Bpl.Expr ProperSubregion(IToken tok, Bpl.Expr e0, Bpl.Expr e1) {
                ...
                return Bpl.Expr.And(
                  translator.FunctionCall(tok, BuiltinFunction.RegionSubRegion, null, e0, e1),
                  Bpl.Expr.Not(translator.FunctionCall(tok, BuiltinFunction.RegionSubRegion, null, e1, e0)));
            }
            ...
            // Yuyan
            public Bpl.Expr TrInRegion(IToken tok, Bpl.Expr elmt, Expression s, Type elmtType) {
                Contract.Requires(tok != null);
                Contract.Requires(elmt != null);
                Contract.Requires(s != null);
                Contract.Requires(elmtType != null);
            ...
            // Yuyan
            public Bpl.Expr GetRthis(IToken tok, Bpl.Expr e) {
                Contract.Requires(tok != null);
                Contract.Requires(e != null);
                Contract.Ensures(Contract.Result<Bpl.Expr>() != null);

                return ReadHeap(tok, HeapExpr, e, predef.RThis(tok));
            }
            ...
            // Yuyan
            public Bpl.Expr GoodRThis(IToken tok, Bpl.Expr o, Type type) {
                Contract.Requires(tok != null);
                Contract.Requires(o != null);
                Contract.Requires(type != null);
                Contract.Ensures(Contract.Result<Bpl.Expr>() != null);

                Bpl.Expr empRegion = translator.FunctionCall(Token.NoToken, BuiltinFunction.RegionEmpty, predef.HeapType);
                Bpl.Expr ret = null;

                Bpl.Expr tmp = null;

                if (type is UserDefinedType) {
                    UserDefinedType ctype = UserDefinedType.DenotesClass(type);
                    if (ctype != null) {
                        ClassDecl cd = (ClassDecl)ctype.ResolvedClass;
                        foreach (MemberDecl member in cd.Members) {
                            if (member is Field) {
                                Field f = (Field)member;

                                if (!f.IsGhost) {
                                    // Region#Singleton(e, f)
                                    Bpl.Expr fexpr = new Bpl.IdentifierExpr(Token.NoToken, translator.GetField(f));
                                    Bpl.Expr single = translator.FunctionCall(Token.NoToken, BuiltinFunction.RegionSingleton, null, o, fexpr);

                                    if (tmp == null) {
                                        tmp = single;
                                    } else {
                                        tmp = translator.FunctionCall(Token.NoToken, BuiltinFunction.RegionUnion, null, tmp, single);
                                    }
                                }
                            }
                        }
                ...
                ret = Bpl.Expr.Eq(GetRthis(Token.NoToken, o), tmp);
            ...
            public Bpl.Expr GoodRef_Class(IToken tok, Bpl.Expr e, UserDefinedType type, bool isNew) {
                ...
                Contract.Ensures(Contract.Result<Bpl.Expr>() != null);
                // Yuyan : added goodrthis
                // return GoodRef_Ref(tok, e, new Bpl.IdentifierExpr(tok, translator.GetClass(type.ResolvedClass)), type.TypeArgs, isNew);
                return Bpl.Expr.And(GoodRef_Ref(tok, e, new Bpl.IdentifierExpr(tok, translator.GetClass(type.ResolvedClass)), type.TypeArgs, isNew),
                           (GoodRThis(tok, e, type)));
            }
        ...
        Bpl.NAryExpr FunctionCall(IToken tok, BuiltinFunction f, Bpl.Type typeInstantiation, params Bpl.Expr[] args) {
            ...
            switch (f) {
                ...
                /*--- Yuyan: Recursive types -----*/
                case BuiltinFunction.RecursiveType:
                    Contract.Assert(args.Length == 2);
                    Contract.Assert(typeInstantiation == null);
                    return FunctionCall(tok, "RecursiveType", Bpl.Type.Bool, args);
                default:
        ...
        internal bool TrSplitExpr(Expression expr, List<SplitExprInfo/*!*/>/*!*/ splits, bool position, int heightLimit, ExpressionTranslator etran) {
            ...
            if (expr is BoxingCastExpr) {
                ...
                if (e.ResolvedOp == UnaryExpr.ResolvedOpcode.Not) {
                    var ss = new List<SplitExprInfo>();
        ...
        public class Substituter {
            ...
            public virtual Expression Substitute(Expression expr) {
                ...
                if (expr is LiteralExpr || expr is WildcardExpr || expr is BoogieWrapper) {
                    ...
                    PatternExpression e = (PatternExpression)expr;
                    return Substitute(e.E.Resolved);
                } else if (expr is DisplayExpression) {
                    DisplayExpression e = (DisplayExpression)expr;
                    ...
                    RegionConstructExpression rce = (RegionConstructExpression)expr;
                    Expression E = Substitute(rce.E);
                    RegionConstructExpression nrce = new RegionConstructExpression(rce.tok, E);
                    List<Region> regionList = new List<Region>();
                    foreach (Region r in rce.RegionList) {
                        regionList.Add(new Region(Substitute(r.Obj), r.Field));
                    }
                    nrce.RegionList = regionList;
                    newExpr = nrce;
                    ...
                    Expression se = Substitute(e.E);
                    if (se != e.E) {
                        UnaryExpr newUnary = new UnaryExpr(expr.tok, e.Op, se);
                        newUnary.ResolvedOp = e.ResolvedOp;
                        newExpr = newUnary;
                    }
                } else if (expr is BinaryExpr) {
                    BinaryExpr e = (BinaryExpr)expr;
                ...
                } else if (expr is FootprintExpression) {
                    var e = (FootprintExpression)expr;
                    FootprintExpression fte = new FootprintExpression(e.tok, Substitute(e.E));
                    for (int i = 0; i < e.Footprint.Count; i++) {
                        fte.Footprint.Add((RegionConstructExpression)Substitute(e.Footprint[i]));
                    }
                    newExpr = fte;
                }
