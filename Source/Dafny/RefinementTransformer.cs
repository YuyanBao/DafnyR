...
namespace Microsoft.Dafny
{
  ...
  public class RefinementTransformer : IRewriter
  {
    ...
    Function CloneFunction(IToken tok, Function f, bool isGhost, List<Expression> moreEnsures, Expression moreBody, Expression replacementBody, bool checkPrevPostconditions, Attributes moreAttributes) {
      ...
      var patterns = f.Patterns.ConvertAll(refinementCloner.ClonePattern); // Yuyan
      ...
      if (f is Predicate) {
        ...
        return new CoPredicate(tok, f.Name, f.IsStatic, tps, f.OpenParen, formals, patterns /* Yuyan */,
          ...
          return new Function(tok, f.Name, f.IsStatic, isGhost, tps, f.OpenParen, formals, patterns /* Yuyan */, refinementCloner.CloneType(f.ResultType),
    ...
    Method CloneMethod(Method m, List<MaybeFreeExpression> moreEnsures, Specification<Expression> decreases, BlockStmt newBody, bool checkPreviousPostconditions, Attributes moreAttributes) {
      ...
      var patterns = m.Patterns.ConvertAll(refinementCloner.ClonePattern);   // Yuyan
      ...
      if (m is Constructor) {
        return new Constructor(new RefinementToken(m.tok, moduleUnderConstruction), m.Name, tps, ins, patterns /* Yuyan */,
          req, mod, ens, decreases, body, refinementCloner.MergeAttributes(m.Attributes, moreAttributes), false);
      } else {
          return new Method(new RefinementToken(m.tok, moduleUnderConstruction), m.Name, m.IsStatic, m.IsGhost, tps, ins, m.Outs.ConvertAll(refinementCloner.CloneFormal), patterns /* Yuyan */,
          req, mod, ens, decreases, body, refinementCloner.MergeAttributes(m.Attributes, moreAttributes), false);
      }
    }
    ...
    IteratorDecl MergeIterator(IteratorDecl nw, IteratorDecl prev) {
      ...
      return new IteratorDecl(new RefinementToken(nw.tok, moduleUnderConstruction), nw.Name, moduleUnderConstruction,
        ...
        nw.SignatureIsOmitted ? prev.Patterns.ConvertAll(refinementCloner.ClonePattern) : nw.Patterns, /* Yuyan */
