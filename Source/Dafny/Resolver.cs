...
namespace Microsoft.Dafny {
    ...
    public class Resolver : ResolutionErrorReporter {
            ...
            foreach (TopLevelDecl d in declarations) {
                ...
                if (d is ModuleDecl) {
                    ...
                    var init = new Constructor(iter.tok, iter.Name, new List<TypeParameter>(), iter.Ins,
                      new List<PatternExpression>() /* Yuyan */,
        ...
        /* Yuyan */
        PatternExpression ClonePattern(PatternExpression pattern) {
            return new PatternExpression(pattern.tok, CloneExpr(pattern.E));
        }
        ...
        Function CloneFunction(IToken tok, Function f, bool isGhost) {
            ...
            var patterns = f.Patterns.ConvertAll(ClonePattern); // Yuyan
            ...
            if (f is Predicate) {
                return new Predicate(tok, f.Name, f.IsStatic, isGhost, tps, f.OpenParen, formals, patterns /* Yuyan */,
                  req, reads, ens, decreases, body, Predicate.BodyOriginKind.OriginalOrInherited, null, false);
            } else if (f is CoPredicate) {
                return new CoPredicate(tok, f.Name, f.IsStatic, tps, f.OpenParen, formals, patterns /* Yuyan */,
                  req, reads, ens, body, null, false);
            } else {
                return new Function(tok, f.Name, f.IsStatic, isGhost, tps, f.OpenParen, formals, patterns /* Yuyan */, CloneType(f.ResultType),
        ...
        Method CloneMethod(Method m) {
            ...
            var patterns = m.Patterns.ConvertAll(ClonePattern);
            ...
            if (m is Constructor) {
                return new Constructor(m.tok, m.Name, tps, ins, patterns /* Yuyan */,
                  req, mod, ens, decreases, null, null, false);
            } else {
                return new Method(m.tok, m.Name, m.IsStatic, m.IsGhost, tps, ins, m.Outs.ConvertAll(CloneFormal), patterns /* Yuyan */,
                  req, mod, ens, decreases, null, null, false);
            }
        }
        ...
        // Yuyan 
        // resolve expression in the form of region expression
        // o <==> region{o.*}
        // this <==> region{this.*}
        // o.f <==> region{o.f}
        // TBD: collection type
        void ResolveRegionExpression(RegionConstructExpression rce) {
            ...
            if (e.Resolved is LiteralExpr) {  // for region{}

            } else if (e.Resolved is WildcardExpr) {
                // nothing to do
            } else if (e.Resolved is FieldSelectExpr) { // region{o.f} 
                var fse = (FieldSelectExpr)e.Resolved;
                t = fse.Obj.Type;
                if (fse.Field.Name == "*") {
                    rce.RegionList = GenerateRegionList(fse.Obj);
                    rce.canUseRThis = false;
                } else {
                    rce.RegionList.Add(new Region(fse.Obj, fse.Field));
                }
            } else if (e.Resolved is ThisExpr) { // region{this} or region{o}
                // use axiom RegionOfRef instead of enumerating all the fileds
                // regionList.Add(new Region(e.Resolved, new Field(Token.NoToken, "*", false, Type.Flexible, null)));
                rce.RegionList = GenerateRegionList(e);
                rce.canUseRThis = false;
            } else if (e.Resolved is IdentifierExpr) {
                rce.RegionList = GenerateRegionList(e);
                rce.canUseRThis = false;    // sometimes not sure what rthis is. we cannot use rthis to generate region
            } else if (e.Resolved is FunctionCallExpr) {
                //***** Cannot resovle it here. rce.RegionLisg with the type List<Region> cannot represent IdentifierExpr *****//
                //FunctionCallExpr f = (FunctionCallExpr)e.Resolved;
                //Contract.Assert(f.Function != null); // follows from the fact that expr has been successfully resolved
                ...
                //for (int i = 0; i < f.Function.Formals.Count; i++) {
                //    substMap.Add(f.Function.Formals[i], f.Args[i].Resolved);
                //}
                //foreach (FrameExpression fe in f.Function.Reads) {
                //    Expression obj = f.Receiver;
                //    if (f.Receiver != null) {
                //        obj = Translator.Substitute(obj, f.Receiver, substMap);
                //    }
                //}

            } else {
                Error(e, "Unexpected expression {0} in region expression resolution.", e.tok.val);
        ...
        // Yuyan 
        List<RegionConstructExpression> ResolveFootprintExpression(Expression expr) {
            Contract.Requires(expr != null);
            Contract.Ensures(Contract.Result<List<RegionConstructExpression>>() != null);
            List<RegionConstructExpression> ret = new List<RegionConstructExpression>();
            if (expr is IdentifierSequence) { 
                ret.Add(new RegionConstructExpression(expr.tok, expr));
            } else if (expr is LiteralExpr) {
                // the footprint is emtpy

            } else if (expr is ThisExpr) {
                ret.Add(new RegionConstructExpression(expr.tok, expr));

            } else if (expr is RegionConstructExpression) { // Yuyan: added to resolve RegionConstructExpression
                ret.Add((RegionConstructExpression)expr);
            } else if (expr is RegionFilterExpression) { // Yuyan: added to resolve RegionFiltertExpression
                RegionFilterExpression fte = (RegionFilterExpression)expr;
                ret.Add(new RegionConstructExpression(fte.tok, fte.regionE));
            } else if (expr is FootprintExpression) {
                FootprintExpression fe = (FootprintExpression) expr;
                ret.AddRange(ResolveFootprintExpression(fe.E));                
            } else if (expr is DisplayExpression) {
                DisplayExpression e = (DisplayExpression)expr;
                foreach (Expression ee in e.Elements) {
                    ret.AddRange(ResolveFootprintExpression(ee));
                }
            } else if (expr is MapDisplayExpr) {
                MapDisplayExpr e = (MapDisplayExpr)expr;
                foreach (ExpressionPair p in e.Elements) {
                    ret.AddRange(ResolveFootprintExpression(p.A));
                    ret.AddRange(ResolveFootprintExpression(p.B));
                }
            } else if (expr is ExprDotName) {
                ret.Add(new RegionConstructExpression(expr.tok, expr));
                ...
                ret.Add(new RegionConstructExpression(e.E.tok, e.E));
            } else if (expr is BinaryExpr) {
                BinaryExpr e = (BinaryExpr)expr;
                ret.Add(new RegionConstructExpression(e.E0.tok, e.E0));
                ret.Add(new RegionConstructExpression(e.E1.tok, e.E1));
        ...
        // Yuyan re-define ResolveFrameExpression
        // resolve original frame expression in a region expression way
        // o <==> region{o.*}
        // this <==> region{this.*}
        // o.f <==> region{o.f} if typeof(o.f) != regiontype
        ...
        void ResolveFrameExpression(FrameExpression fe, string kind) {
            ...
            if (!(t is RegionType)) {
                fe.ResolvedExpression = new RegionConstructExpression(fe.E.tok, fe.E);
                ResolveExpression(fe.ResolvedExpression, false);
            } else {
                fe.ResolvedExpression = fe.E;
                ResolveExpression(fe.ResolvedExpression, false);
            }
        }
        ...
        public bool UnifyTypes(Type a, Type b) {
            ...
            if (a is ObjectType || b is ObjectType) {  // TODO: remove this temporary hack
                ...
                if (other is BoolType || other is IntType || other is SetType || other is SeqType || other is RegionType /* Yuyan: not know why*/ || other.IsDatatype) {
                    return false;
                }
            ...
            if (a is BoolType) {
                ...
                return b is IntType;
            } else if (a is RegionType) { // Yuyan: type check for regiontype
                return b is RegionType;
        ...
        bool AssignProxy(TypeProxy proxy, Type t) {
            ...
            if (proxy == t) {
                ...
                if (t is IntType || t is SetType || t is MultiSetType || (opProxy.AllowSeq && t is SeqType) || t is RegionType /* Yuyan added*/) {
        ...
        void ResolveCallStmt(CallStmt s, bool specContextOnly, ICodeContext codeContext, Type receiverType) {
            ...
            if (member == null) {
                // Yuyan
                // error has already been reported by ResolveMember
                // otherwise it is a wildcard expression, which is not allowed here. An error should report
                Error(s.Tok, "Wildcard expression is not allowed in a call statement");
            } else if (member is Method) {
        ...
        MemberDecl ResolveMember(IToken tok, Type receiverType, string memberName, out NonProxyType nptype) {
            ...
            if (ctype != null) {
                ...
                Contract.Assert(ctype.TypeArgs.Count == cd.TypeArgs.Count);  // follows from the fact that ctype was resolved
                MemberDecl member = null; // Yuyan: initialized it to null
                if (memberName == "*") { // Yuyan:  wildcard
                    nptype = ctype;
                    return null;
                } else if (!classMembers[cd].TryGetValue(memberName, out member)) {
            ...
            if (dtd != null) {
                ...
                if (memberName == "*") { // Yuyan 
                    return null;
                } else if (!datatypeMembers[dtd].TryGetValue(memberName, out member)) {
        ...
        void ResolveExpression(Expression expr, bool twoState, List<IVariable> matchVarContext) {
            ...
            if (expr is ParensExpression) {
                ...
                if (e.E.Type == null) {
                    scope.Push(e.E.tok.val, new Formal(e.E.tok, e.E.tok.val, Type.Flexible, false, true));    // not need to check the duplicity. it is done somewhere else
                } else {
                    scope.Push(e.E.tok.val, new Formal(e.E.tok, e.E.tok.val, e.E.Type, false, true));
                    e.Type = e.E.Type;
                    e.E = new IdentifierExpr(e.E.tok, e.E.tok.val); // Resolve it again;
                }

                ResolveExpression(e.E, twoState);
                // the type is resolved somewhere else
                // give a flexible type temporarily
                if (e.Type == null) {
                    e.Type = Type.Flexible;
                }
                ...
                if (!moduleInfo.TopLevels.TryGetValue(ce.ClassName, out d)) {
                    Error(expr.tok, "Undeclared datatype: {0}", ce.ClassName);
                } else if (d is AmbiguousTopLevelDecl) {
                    Error(expr.tok, "The name {0} ambiguously refers to a type in one of the modules {1}", ce.ClassName, ((AmbiguousTopLevelDecl)d).ModuleNames());
                } else if (d is ClassDecl) {
                    ce.ClsDecl = (ClassDecl)d;
                } else {
                    Error(expr.tok, "Expected class name: {0}", ce.ClassName);
                }
                ...
                ResolveExpression(rce.E, twoState);
                Type t = rce.E.Type;
                Contract.Assert(t != null);  // follows from postcondition of ResolveExpression
                ...
                ResolveExpression(rfe.regionE, twoState);
                foreach (Expression filter in rfe.filterList) {
                    ResolveExpression(filter, twoState);
                }
                rfe.filterType = ResolveFilterType(rfe.filterList);
                expr.Type = new RegionType();
            }else if (expr is FootprintExpression){  // Yuyan
                FootprintExpression fe = (FootprintExpression)expr;
                ResolveExpression(fe.E, twoState);
                fe.Footprint = ResolveFootprintExpression(fe.E);
...
#endif
                if (member == null) {
                    // Yuyan: it must be a wildcard expression
                    // otherwise error has already been reported by ResolveMember
                    e.Field = new Field(expr.tok, expr.tok.val, false, Type.Flexible, null);
                } else if (!(member is Field)) {
                ...
                switch (e.Op) {
                    ...
                    case UnaryExpr.Opcode.SetChoose:
                        ...
                        expr.Type = elType;
                        break;
                    case UnaryExpr.Opcode.Length:
                        if (!UnifyTypes(e.E.Type, new SeqType(new InferredTypeProxy())) &&
                            !UnifyTypes(e.E.Type, new RegionType())) {
                            Error(expr, "length operator expects a sequence or region argument (instead got {0})", e.E.Type);
                        }
                        expr.Type = Type.Int;
                        break;
                    default:
                ...
                e.ResolvedOp = ResolveOp(e.Op, e.E.Type);
                ...
                // Yuyan: resolve pattern expression here
                ResolvePatternExpression(e);
                switch (e.Op) {
                    ...
                    case BinaryExpr.Opcode.Disjoint:
                        ...
                        if (!UnifyTypes(e.E0.Type, new SetType(new InferredTypeProxy())) &&
                            ...
                            !UnifyTypes(e.E0.Type, new RegionType())/* Yuyan*/) {
                            Error(expr, "arguments must be of a [multi]set, map type or region type (got {0})", e.E0.Type);
                        }
                        expr.Type = Type.Bool;
                        break;
                ...
                BuiltInFunctionExpression builtInFunExpr = (BuiltInFunctionExpression) expr;

                Contract.Assert(builtInFunExpr.Args != null); // follow the invariant of e

                foreach (Expression arg in builtInFunExpr.Args)
                {
                    ResolveExpression(arg, twoState, matchVarContext);
                }
                ResolveBuiltInFunctionExpression(builtInFunExpr);
                expr.Type = builtInFunExpr.Type;
        ...
        void ResolveBuiltInFunctionExpression(BuiltInFunctionExpression e)
        {
            Contract.Requires(e != null);

            Contract.Assert(e.FunctionName != null && e.Args != null); // follow the invariant of e

            if (e.FunctionName == "IsRecursiveType")
            {
                if (e.Args.Count != 1)
                {
                    Error(e.tok, "Wrong numbers of function {0} arguments, expected: 1, actural:{1}", e.FunctionName, e.Args.Count);
                }
                else
                {
                    if (!(e.Args[0].Resolved.Type is UserDefinedType))
                    {
                        Error(e.tok, "UserDeinfedType expected, got {0}", e.Args[0].Resolved.Type.ToString());
                    }
                    else
                    {
                        e.Type = Type.Bool;
            ...
            {
                Error(e.tok, "undefined function {0}", e.FunctionName);
            }
        ...
        void ResolvePatternExpression(Expression e) {
            Contract.Requires(e != null);

            if (e is BinaryExpr) {
                BinaryExpr be = (BinaryExpr)e;
                if (be.E0 is PatternExpression) {
                    ((PatternExpression)be.E0).Type = be.E1.Type;
                } else if (be.E1 is PatternExpression) {
                    ((PatternExpression)be.E1).Type = be.E0.Type;
                }
            }
        }
        ...
        // 2: filter by instance and field name
        // 3: filter by classname
        // 4: filter by classname and field name
        int ResolveFilterType(List<Expression> filters) {
            Contract.Requires(filters != null);
            int type = -1;
            if (filters.Count == 0) {
                type = 0;
            } else {
                Contract.Assert(filters.Count > 0);
                if (filters[0] is ClassExpression) {
                    ClassExpression ce = (ClassExpression)filters[0];
                    if (filters.Count == 1) {
                        type = 3;
                    } else {
                        Contract.Assert(filters.Count > 1);
                        for (int i = 1; i < filters.Count; i++) {
                            if (filters[i] is FieldExpression) {
                                FieldExpression fe = (FieldExpression)filters[i];
                                ClassDecl cd = ((ClassExpression)filters[0]).ClsDecl;
                                foreach (MemberDecl md in cd.Members) {
                                    if (md is Field) {
                                        Field f = (Field)md;
                                        if (f.Name == fe.FieldName) {
                                            fe.Field = f;
                                            break;
                                        }
                                    }
                                ...
                                if (fe.Field == null) {
                                    Error(fe.tok, "member {0} is in the class {1} does not refer to a field.", fe.FieldName, cd.Name);
                                }
                            } else {
                                Error(filters[i], "FieldExpression expected, instead of {0}.", filters[i]);
                            }
                        }

                        type = 4;
                    }
                } else { // an object instance
                    if (filters.Count == 1) {
                        type = 1;
                    } else {
                        Contract.Assert(filters.Count > 1);
                        for (int i = 1; i < filters.Count; i++) {
                            if (filters[i] is FieldExpression) {
                                FieldExpression fe = (FieldExpression)filters[i];
                                NonProxyType nptype;
                                MemberDecl member = ResolveMember(filters[0].tok, filters[0].Type, fe.tok.val, out nptype);
                                UserDefinedType ctype = (UserDefinedType)nptype;  // correctness of cast follows from the DenotesClass test above
                                if (member == null) {
                                    // Yuyan: a wildcard expression is not allowed here. An error should report.
                                    // otherwise error has already been reported by ResolveMember
                                    Error(filters[i], "a wildcard expression is not allowed in a region filter expression");
                                } else if (!(member is Field)) {
                                    Error(filters[i], "member {0} in type {1} does not refer to a field", fe.tok.val, filters[0].Type);
                                } else {
                                    Contract.Assert(ctype != null && ctype.ResolvedClass != null);  // follows from postcondition of ResolveMember
                                    fe.Field = (Field)member;
                                }
        ...
        private void ResolveDatatypeValue(bool twoState, DatatypeValue dtv, DatatypeDecl dt) {
            ...
            foreach (Expression arg in dtv.Arguments) {
                ...
                if (arg is PatternExpression) {   // Yuyan: resove pattern expression here
                    PatternExpression pe = (PatternExpression)arg;
                    pe.Type = formal.Type;
                    ResolveType(pe.tok, pe.Type, dt.TypeArgs, true);
                    ResolveExpression(pe, twoState);
                }
        ...
        CallRhs ResolveFunctionCallExpr(FunctionCallExpr e, bool twoState, bool allowMethodCall) {
            ...
            if (member == null) {
                ...
                // error has already been reported by ResolveMember
                // otherwise it is a wildcard expression, which is not allowed here. An error should report
                Error(e.tok, "Wildcard expression is not allowed in a function call expression");
            } else if (member is Method) {
                if (allowMethodCall) {
                    // it's a method
                ...
                if (function.Formals.Count != e.Args.Count) {
                    ...
                    for (int i = 0; i < function.Formals.Count; i++) {
                        ...
                        if (farg is PatternExpression) {
                            PatternExpression pe = (PatternExpression)farg;
                            pe.E.Type = s;
                        }
            ...
            } else if (sig.StaticMembers.TryGetValue(id.val, out member)) // try static members of the current module too.
      {
        ...
        Expression/*?*/ ResolvePredicateOrField(IToken tok, Expression obj, string suffixName) {
            ...
            if (member == null) {
                // Yuyan
                // // error has already been reported by ResolveMember if it is a valid filedname
                // otherwise it is a wildcard expression, feed * 
                return new FieldSelectExpr(tok, obj, suffixName); // suffixName must equal to "="
            ...
            if (!e.SelectOne)  // require sequence or array
      {
        ...
        // Yuyan
        UnaryExpr.ResolvedOpcode ResolveOp(UnaryExpr.Opcode op, Type operandType) {
            Contract.Requires(operandType != null);
            switch (op) {
                case UnaryExpr.Opcode.Length:
                    if (operandType is SeqType) {
                        return UnaryExpr.ResolvedOpcode.SeqLength;
                    } else {
                        return UnaryExpr.ResolvedOpcode.RegionLength;
                    }
                case UnaryExpr.Opcode.Not:
                    return UnaryExpr.ResolvedOpcode.Not;
                case UnaryExpr.Opcode.SetChoose:
                    return UnaryExpr.ResolvedOpcode.SetChoose;
                default:
                    Contract.Assert(false); throw new cce.UnreachableException();  // unexpected operator
            }
        }
        ...
        BinaryExpr.ResolvedOpcode ResolveOp(BinaryExpr.Opcode op, Type operandType) {
            ...
            switch (op) {
                ...
                case BinaryExpr.Opcode.Eq:
                    ...
                    } else if (operandType is RegionType) { // Yuyan
                        return BinaryExpr.ResolvedOpcode.RegionEq;
                    } else {
                ...
                case BinaryExpr.Opcode.Neq:
                    if (operandType is SetType) {
                        ...
                        return BinaryExpr.ResolvedOpcode.RegionNeq;
                    } else {
                ...
                case BinaryExpr.Opcode.Disjoint:
                    ...
                    } if (operandType is RegionType) {   // Yuyan
                        return BinaryExpr.ResolvedOpcode.RegionDisjoint;
                    } else {
                ...
                case BinaryExpr.Opcode.Lt:
                    if (operandType.IsIndDatatype || operandType is DatatypeProxy) {
                        ...
                        return BinaryExpr.ResolvedOpcode.RegionSubRegion;
                    } else {
                ...
                case BinaryExpr.Opcode.Le:
                    if (operandType is SetType) {
                        ...
                        return BinaryExpr.ResolvedOpcode.RegionSubOrEqualRegion;
                    } else {
                ...
                case BinaryExpr.Opcode.Add:
                    if (operandType is SetType) {
                        ...
                        return BinaryExpr.ResolvedOpcode.RegionUnion;
                    } else {
                ...
                case BinaryExpr.Opcode.Sub:
                    if (operandType is SetType) {
                        ...
                        return BinaryExpr.ResolvedOpcode.RegionDifference;
                    } else {
                ...
                case BinaryExpr.Opcode.Mul:
                    if (operandType is SetType) {
                        ...
                        return BinaryExpr.ResolvedOpcode.RegionIntersection;
                    } else {
                ...
                case BinaryExpr.Opcode.In:
                    if (operandType is SetType) {
                        ...
                        return BinaryExpr.ResolvedOpcode.InRegion;
                    } else {
                ...
                case BinaryExpr.Opcode.NotIn:
                    if (operandType is SetType) {
                        ...
                        return BinaryExpr.ResolvedOpcode.NotInRegion;
                    } else {
