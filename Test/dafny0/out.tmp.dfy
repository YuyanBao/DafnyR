...
// Command Line Options: /compile:0 /dprint:out.tmp.dfy -nologo -logPrefix:dafny0 LetExpr.dfy
// LetExpr.dfy

method M0(n: int)
  requires var f := 100; n < f;
  requires var t, f := true, false; (t && f) || n < 100;
{
  assert n < 200;
  assert 0 <= n;
}
...
{
  assert Fib(2) + Fib(4) == Fib(0) + Fib(1) + Fib(2) + Fib(3);
  assert var x, y := Fib(8), Fib(11); assume x == 21; assert Fib(7) == 3 ==> Fib(9) == 24; assume Fib(1000) == 1000; assume Fib(9) - Fib(8) == 13; assert Fib(9) <= Fib(10); y == 89;
  assert Fib(1000) == 1000;
  ...
  ensures var oi := old(index); oi == index ==> a[oi] == 21 && old(a[oi]) == 19;
{
  ...
  while (i < a.Length)
    ...
    invariant (0 <= j < i && old(a[j]) == 19) || (j == 0 - 1 && exists k :: i <= k < a.Length && a[k] == 19);
  {
    if (a[i] == 19) {
      j := i;
    }
  ...
  ensures 1 <= Fib(n);
{
  if n < 3 then
    ...
    var x := Theorem2(n - 2); var y := Theorem2(n - 1); x + y
}
  ...
  ensures 1 <= Fib(n);
{
  if n < 3 then
    ...
    var x := Theorem3(n - 2); var y := Theorem3(n - 1); 5
}
