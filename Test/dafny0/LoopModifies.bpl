...
// Command Line Options: C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy /print C:\Projects\Boogie\Test\dafny0\LoopModifies.bpl /smoke /compile 0
...
procedure CheckWellformed$$_module.__default.Testing7(this: ref where this != null && read($Heap, this, alloc) && dtype(this) == class._module.__default, a#0: ref where a#0 == null || (read($Heap, a#0, alloc) && dtype(a#0) == class._System.array && TypeParams(a#0, 0) == class._System.int), b#1: ref where b#1 == null || (read($Heap, b#1, alloc) && dtype(b#1) == class._System.array && TypeParams(b#1, 0) == class._System.int));
  free requires 0 == $ModuleContextHeight && $InMethodContext;
  modifies $Heap, $Tick;



implementation CheckWellformed$$_module.__default.Testing7(this: ref, a#0: ref, b#1: ref)
{
  var $_Frame: <beta>[ref,Field beta]bool;

    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: $o != null && read($Heap, $o, alloc) ==> $o == a#0 || $o == b#1);
    if (a#0 != null)
    {
        assert a#0 != null;
    }

    assume a#0 != null && _System.array.Length(a#0) > 0;
    if (b#1 != null)
    {
        assert b#1 != null;
    }

    assume b#1 != null && _System.array.Length(b#1) > 0;
    assume a#0 != b#1;
    havoc $Heap;
    assume (forall<alpha> $o: ref, $f: Field alpha :: { read($Heap, $o, $f) } $o != null && read(old($Heap), $o, alloc) ==> read($Heap, $o, $f) == read(old($Heap), $o, $f) || $o == a#0 || $o == b#1);
    assume $HeapSucc(old($Heap), $Heap);
}



procedure _module.__default.Testing7(this: ref where this != null && read($Heap, this, alloc) && dtype(this) == class._module.__default, a#0: ref where a#0 == null || (read($Heap, a#0, alloc) && dtype(a#0) == class._System.array && TypeParams(a#0, 0) == class._System.int), b#1: ref where b#1 == null || (read($Heap, b#1, alloc) && dtype(b#1) == class._System.array && TypeParams(b#1, 0) == class._System.int));
  free requires 0 == $ModuleContextHeight && $InMethodContext;
  requires a#0 != null;
  requires _System.array.Length(a#0) > 0;
  requires b#1 != null;
  requires _System.array.Length(b#1) > 0;
  requires a#0 != b#1;
  modifies $Heap, $Tick;
  // frame condition
  free ensures (forall<alpha> $o: ref, $f: Field alpha :: { read($Heap, $o, $f) } $o != null && read(old($Heap), $o, alloc) ==> read($Heap, $o, $f) == read(old($Heap), $o, $f) || $o == a#0 || $o == b#1);
  // boilerplate
  free ensures $HeapSucc(old($Heap), $Heap);



implementation _module.__default.Testing7(this: ref, a#0: ref, b#1: ref)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var i#2: int;
  var $rhs#0: BoxType;
  ...
  var $rhs#1: BoxType;
    ...
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: $o != null && read($Heap, $o, alloc) ==> $o == a#0 || $o == b#1);
    // ----- var-declaration statement ----- C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(110,8)
    // ----- assignment statement ----- C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(110,10)
    assume true;
    assume true;
    i#2 := 0;
    assume {:captureState "C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(110,10)"} true;
    // ----- assignment statement ----- C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(111,9)
    assert b#1 != null;
    assert 0 <= 0 && 0 < _System.array.Length(b#1);
    assume true;
    assert $_Frame[b#1, IndexField(0)];
    assume true;
    $rhs#0 := $Box(4);
    $Heap := update($Heap, b#1, IndexField(0), $rhs#0);
    assume $IsGoodHeap($Heap);
    assume {:captureState "C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(111,9)"} true;
    // ----- while statement ----- C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(112,4)
    assert (forall<alpha> $o: ref, $f: Field alpha :: $o != null && read($Heap, $o, alloc) && $o == a#0 ==> $_Frame[$o, $f]);
    #_Frame#0 := (lambda<alpha> $o: ref, $f: Field alpha :: $o != null && read($Heap, $o, alloc) ==> $o == a#0);
    $PreLoopHeap0 := $Heap;
    $decr0$init$0 := 10 - i#2;
    havoc $w0;
    while (true)
      free invariant $w0 ==> 0 <= i#2 ==> true;
      invariant $w0 ==> 0 <= i#2;
      invariant $w0 ==> i#2 <= 10;
      free invariant (forall<alpha> $o: ref, $f: Field alpha :: { read($Heap, $o, $f) } $o != null && read(old($Heap), $o, alloc) ==> read($Heap, $o, $f) == read($PreLoopHeap0, $o, $f) || $o == a#0 || $o == b#1);
      ...
      free invariant 10 - i#2 <= $decr0$init$0 && (10 - i#2 == $decr0$init$0 ==> true);
    {
        if (!$w0)
        {
            if (0 <= i#2)
        ...
        assume {:captureState "C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(112,4): loop entered"} true;
        $decr0$0 := 10 - i#2;
        // ----- assignment statement ----- C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(117,12)
        assert a#0 != null;
        assert 0 <= 0 && 0 < _System.array.Length(a#0);
        assume true;
        assert #_Frame#0[a#0, IndexField(0)];
        assume true;
        $rhs#1 := $Box(i#2);
        $Heap := update($Heap, a#0, IndexField(0), $rhs#1);
        assume $IsGoodHeap($Heap);
        assume {:captureState "C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(117,12)"} true;
        // ----- assignment statement ----- C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(118,9)
        assume true;
        assume true;
        i#2 := i#2 + 1;
        assume {:captureState "C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(118,9)"} true;
        assert 0 <= $decr0$0 || 10 - i#2 == $decr0$0;
        assert 10 - i#2 < $decr0$0;
        assume 0 <= i#2 ==> true;
    }

    assume {:captureState "C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(112,4): loop exit"} true;
    // ----- assert statement ----- C:\Projects\Boogie\Test\dafny0\LoopModifies.dfy(121,4)
    assert b#1 != null;
    assert {:subsumption 0} 0 <= 0 && 0 < _System.array.Length(b#1);
    assume true;
    assert $Unbox(read($Heap, b#1, IndexField(0))): int == 4;
}
